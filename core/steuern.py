...
class SteuerRechner:
    ...
    def berechne_einkommensteuer(self, zve: float, jahr: int = None) -> float:
        if jahr is None:
            jahr = datetime.now().year
        p = jahresparameter(jahr)
        zve = max(0, int(zve))
        if zve <= p.est_grundfreibetrag:
            return 0.0
        if zve <= p.est_zone2_grenze:
            y = (zve - p.est_grundfreibetrag) / 10000
            return (914.51 * y + 1400) * y
        if zve <= p.est_zone3_grenze:
            z = (zve - p.est_zone2_grenze) / 10000
            return (173.10 * z + 2397) * z + 1034.87
        if zve <= p.est_zone4_grenze:
    ...
    def berechne_nettoaufwand_entgeltumwandlung(
        ...
        steuer_vorher = self.berechne_einkommensteuer(brutto_monat * 12, jahr)
        steuer_nachher = self.berechne_einkommensteuer((brutto_monat - abzugsbetrag) * 12, jahr)
    ...
    def berechne_netto_aus_rentengruppe(
        ...
        gemeinsame_steuer = self.berechne_einkommensteuer(gesamt_bemessung * 12, jahr) / 12
        netto_werte = []
