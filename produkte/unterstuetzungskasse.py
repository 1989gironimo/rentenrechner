...
class Unterstuetzungskasse(RentenProdukt):
    def __init__(self,
                 ...
                 kosten: dict = None,                      
                 kosten_renditeminderung_prozent: float = 0.0,  # Kosten auf Kapital/Rendite p.a.
                 kosten_beitrag_prozent: float = 0.0,          # Kosten in % vom mtl. Beitrag
                 fixe_verwaltungskosten_monat: float = 0.0,    # Feste mtl. Gebühr in €
                 psvag_promille: float = 0.0,                  # Nur zur Info für AG-Aufwand
        ...
        # Sauber getrennte Kostenstruktur
    ...
    def _berechne_initiale_jahre(self, start_datum: str, beschaeftigungs_start_datum: str) -> int:
        d_beschaeftigung = datetime.strptime(beschaeftigungs_start_datum, "%Y-%m-%d")
        d_start = datetime.strptime(start_datum, "%Y-%m-%d")
        
        if d_start < d_beschaeftigung:
            return 0
            
        diff_tage = (d_start - d_beschaeftigung).days
        return int(diff_tage // 365.25)
    ...
    def berechne_endkapital_nominal(self, profil: NutzerProfil) -> float:
        ...
        # 1. Netto-Rendite p.a. ermitteln (Bruttorendite minus kapitalspezifische Kosten)
        ...
        for m in range(1, int(monate) + 1):
            ...
            # 2. Beitragsbasierte Abzüge berechnen (% vom Beitrag + fixer €-Betrag)
            ...
            # 3. Zinseszins anwenden und Netto-Beitrag anlegen
            # (Hinweis: PSVAG wird HIER NICHT abgezogen, da vom AG getragen)
