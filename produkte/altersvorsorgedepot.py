...
class AltersvorsorgeDepot(RentenProdukt):
    def __init__(
        ...
        anzahl_kinder: int = 0,
        kindergeburtsjahre: Optional[List[int]] = None,
        ...
        **kwargs,
        ...
        self.kindergeburtsjahre = list(kindergeburtsjahre or [])
        ...
        if staatliche_foerderung_prozent:
            warnings.warn(
                "staatliche_foerderung_prozent wird ignoriert; es gilt die gesetzliche Zulagenstaffel.",
                UserWarning,
                stacklevel=2,
            )
        if self.kindergeburtsjahre and anzahl_kinder != len(self.kindergeburtsjahre):
            warnings.warn(
                "anzahl_kinder und kindergeburtsjahre widersprechen sich; Geburtsjahre werden verwendet.",
                UserWarning,
                stacklevel=2,
            )
    ...
    def name(self) -> str:
        return "Altersvorsorgedepot (ab 2027)"
    ...
    def _alter_zum_start(self, profil: NutzerProfil) -> int:
        ...
        return start.year - geburt.year - ((start.month, start.day) < (geburt.month, geburt.day))

    def _berechne_grundzulage(self, jahres_beitrag: float, jahr_index: int, profil: NutzerProfil) -> float:
    ...
    def _berechne_kinderzulage(self, jahres_beitrag: float, kalenderjahr: int) -> float:
        if jahres_beitrag < MIN_BEITRAG_KINDERZULAGE:
            return 0.0

        if self.kindergeburtsjahre:
            return sum(
                KINDERZULAGE_PRO_KIND
                for geburt in self.kindergeburtsjahre
                if 0 <= kalenderjahr - geburt < 18
            )

        return self.anzahl_kinder * KINDERZULAGE_PRO_KIND

    def _berechne_jahrliche_zulage(self, profil: Optional[NutzerProfil] = None, jahr_index: int = 0) -> float:
        jahres_beitrag = self.monatlicher_eigenbeitrag * 12
        if self.staatliche_foerderung_prozent > 0.0:
            return jahres_beitrag * self.staatliche_foerderung_prozent / 100.0

        if profil is None:
            gefoerderter_beitrag = min(jahres_beitrag, AV_SUBSIDIZED_CAP)
            zulage = min(gefoerderter_beitrag, 360.0) * 0.50
            if gefoerderter_beitrag > 360.0:
                zulage += (gefoerderter_beitrag - 360.0) * 0.25
            if jahres_beitrag >= MIN_BEITRAG_KINDERZULAGE:
                zulage += self.anzahl_kinder * KINDERZULAGE_PRO_KIND
            return zulage

        kalenderjahr = self._parse_start_datum().year + jahr_index
        grundzulage = self._berechne_grundzulage(jahres_beitrag, jahr_index, profil)
        kinderzulage = self._berechne_kinderzulage(jahres_beitrag, kalenderjahr)
    ...
    def berechne_monatliche_foerderung(self, *args) -> float:
        if len(args) == 1:
            ...
            if isinstance(arg, NutzerProfil):
                return self._berechne_jahrliche_zulage(arg, 0) / 12
            return self._berechne_jahrliche_zulage() / 12
    ...
    def berechne_monatliche_details(
        ...
        foerderung = self._berechne_jahrliche_zulage(profil, jahr_index) / 12
        beitrag = self.monatlicher_eigenbeitrag
        return beitrag, 0.0, foerderung, beitrag + foerderung

    def berechne_monatliche_steuerersparnis(self, profil: NutzerProfil, steuer_rechner: SteuerRechner) -> float:
        zulage = self._berechne_jahrliche_zulage(profil, 0) / 12
    ...
    def berechne_monatliche_netto_eigenleistung(
        ...
        return max(0.0, self.monatlicher_eigenbeitrag)
    ...
    def berechne_endkapital_nominal(self, profil: NutzerProfil) -> float:
        ...
        for jahr in range(volle_jahre):
            zulage = self._berechne_jahrliche_zulage(profil, jahr)
            subsidized_beitrag = min(annualer_beitrag, AV_SUBSIDIZED_CAP)
            ueberzahlung_beitrag = max(0.0, annualer_beitrag - AV_SUBSIDIZED_CAP)
            monatlicher_subsidized = subsidized_beitrag / 12.0 + zulage / 12.0
            monatlicher_ueberzahlung = ueberzahlung_beitrag / 12.0

            for _ in range(12):
                subsidized_capital = subsidized_capital * (1 + monatlicher_zins) + monatlicher_subsidized
                ueberzahlung_capital = ueberzahlung_capital * (1 + monatlicher_zins) + monatlicher_ueberzahlung

        if rest_monate > 0:
            zulage = self._berechne_jahrliche_zulage(profil, volle_jahre) * (rest_monate / 12.0)
            subsidized_beitrag = min(annualer_beitrag, AV_SUBSIDIZED_CAP) * (rest_monate / 12.0)
            ueberzahlung_beitrag = max(0.0, annualer_beitrag - AV_SUBSIDIZED_CAP) * (rest_monate / 12.0)
            if rest_monate > 0:
                monatlicher_subsidized = subsidized_beitrag / rest_monate + zulage / rest_monate
                monatlicher_ueberzahlung = ueberzahlung_beitrag / rest_monate
            else:
                monatlicher_subsidized = 0.0
                monatlicher_ueberzahlung = 0.0
